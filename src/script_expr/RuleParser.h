...
#ifndef __RULEPARSER_H__
#define __RULEPARSER_H__
...
protected:
    void _compile(const char *code, const char **rest, TokenMap parent_scope)
    {
        ...
        if (it != stmt_map.end())
        {
            ...
            _rulestmt = it->second();
            _rulestmt->compile(code + name.size(), rest, parent_scope);
            return;
        }
...
struct RuleParserStartup
{
    RuleParserStartup()
    {
        // init block statement inside parser 
        BlockStatement::statementMap_t &statementMap = BlockStatement::statementMap();
